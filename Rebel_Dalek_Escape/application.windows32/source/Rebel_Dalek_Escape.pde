...
void draw() {
  ...
  if (inPlayMode)
  {
    ...
    if (randomExterminate <= 0.009) {
      exterminate.play();
    }
    if (randomExterminate >= 0.01) {
      if (randomExterminate <= 0.015) {
        message2.play();
      }
    }
...
void drawGuide() { // circle inside pipe gap
  stroke(255);
  strokeWeight(1);
  int i=0;
